...
const ProfileForm = ({ initialData, onSuccess }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="card p-5 sm:p-6 mb-6">
            ...
            {
                error && (
                    <div className="bg-rose-50 text-rose-500 p-4 rounded-xl text-sm border border-rose-200 mb-6 flex items-start gap-3">
            ...
            }
            {
                message && (
                    <div className="bg-emerald-50 text-emerald-700 p-4 rounded-xl text-sm border border-emerald-200 mb-6 flex items-start gap-3">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1.5 shrink-0" />{error}
                    </div>
                )
            }
                ...
                {initialData.isDeleted ? (
                    <div className="pt-2">
                        <div className="p-4 bg-rose-50 border border-rose-200 rounded-xl text-center ">
                            ...
                            <p className="text-rose-600 mt-0.5 text-sm">You can no longer update your profile</p>
                        </div>
                    </div>
                ) : (
